...
func (l *StructureLoader) LoadThunk(key uint64) func() (*athena.Structure, error) {
	...
	if it, ok := l.cache[key]; ok {
		l.mu.Unlock()
		return func() (*athena.Structure, error) {
			return it, nil
		}
	}
	if l.batch == nil {
...
func (l *StructureLoader) LoadAll(keys []uint64) ([]*athena.Structure, []error) {
	...
	structures := make([]*athena.Structure, len(keys))
...
func (b *structureLoaderBatch) startTimer(l *StructureLoader) {
	time.Sleep(l.wait)
	l.mu.Lock()
...
func (b *structureLoaderBatch) end(l *StructureLoader) {
