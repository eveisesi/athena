...
type CharacterCorporationHistoryLoader struct {
	...
	cache map[uint][]*athena.CharacterCorporationHistory
...
type characterCorporationHistoryLoaderBatch struct {
	keys    []uint
	data    [][]*athena.CharacterCorporationHistory
	error   []error
	closing bool
	done    chan struct{}
}
...
func (l *CharacterCorporationHistoryLoader) Load(key uint) ([]*athena.CharacterCorporationHistory, error) {
...
func (l *CharacterCorporationHistoryLoader) LoadThunk(key uint) func() ([]*athena.CharacterCorporationHistory, error) {
	l.mu.Lock()
	if it, ok := l.cache[key]; ok {
		l.mu.Unlock()
		return func() ([]*athena.CharacterCorporationHistory, error) {
			return it, nil
		}
	}
	if l.batch == nil {
	...
	return func() ([]*athena.CharacterCorporationHistory, error) {
		...
		var data []*athena.CharacterCorporationHistory
...
func (l *CharacterCorporationHistoryLoader) LoadAll(keys []uint) ([][]*athena.CharacterCorporationHistory, []error) {
	results := make([]func() ([]*athena.CharacterCorporationHistory, error), len(keys))

	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}

	characterCorporationHistorys := make([][]*athena.CharacterCorporationHistory, len(keys))
...
func (l *CharacterCorporationHistoryLoader) Prime(key uint, value []*athena.CharacterCorporationHistory) bool {
	l.mu.Lock()
	var found bool
	if _, found = l.cache[key]; !found {
		// make a copy when writing to the cache, its easy to pass a pointer in from a loop var
		// and end up with the whole cache pointing to the same value.
		cpy := make([]*athena.CharacterCorporationHistory, len(value))
		copy(cpy, value)
		l.unsafeSet(key, cpy)
	}
	l.mu.Unlock()
...
func (l *CharacterCorporationHistoryLoader) Clear(key uint) {
	l.mu.Lock()
	delete(l.cache, key)
	l.mu.Unlock()
}

func (l *CharacterCorporationHistoryLoader) unsafeSet(key uint, value []*athena.CharacterCorporationHistory) {
	if l.cache == nil {
		l.cache = map[uint][]*athena.CharacterCorporationHistory{}
	}
	l.cache[key] = value
}
...
func (b *characterCorporationHistoryLoaderBatch) keyIndex(l *CharacterCorporationHistoryLoader, key uint) int {
