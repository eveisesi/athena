...
type CategoryLoader struct {
	...
	cache map[uint]*athena.Category
...
func (l *CategoryLoader) LoadThunk(key uint) func() (*athena.Category, error) {
	...
	if it, ok := l.cache[key]; ok {
		l.mu.Unlock()
		return func() (*athena.Category, error) {
			return it, nil
		}
	}
	if l.batch == nil {
...
func (l *CategoryLoader) LoadAll(keys []uint) ([]*athena.Category, []error) {
	results := make([]func() (*athena.Category, error), len(keys))

	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}

	categorys := make([]*athena.Category, len(keys))
...
func (l *CategoryLoader) unsafeSet(key uint, value *athena.Category) {
	if l.cache == nil {
		l.cache = map[uint]*athena.Category{}
	}
	l.cache[key] = value
}
...
func (b *categoryLoaderBatch) keyIndex(l *CategoryLoader, key uint) int {
