...
func (l *GroupLoader) LoadThunk(key uint) func() (*athena.Group, error) {
	...
	if it, ok := l.cache[key]; ok {
		l.mu.Unlock()
		return func() (*athena.Group, error) {
			return it, nil
		}
	}
	if l.batch == nil {
...
func (l *GroupLoader) LoadAll(keys []uint) ([]*athena.Group, []error) {
	results := make([]func() (*athena.Group, error), len(keys))

	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}

	groups := make([]*athena.Group, len(keys))
...
func (l *GroupLoader) unsafeSet(key uint, value *athena.Group) {
	if l.cache == nil {
		l.cache = map[uint]*athena.Group{}
	}
	l.cache[key] = value
}
...
func (b *groupLoaderBatch) keyIndex(l *GroupLoader, key uint) int {
