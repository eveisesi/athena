...
type CorporationLoader struct {
	...
	cache map[uint]*athena.Corporation
...
func (l *CorporationLoader) LoadThunk(key uint) func() (*athena.Corporation, error) {
	...
	if it, ok := l.cache[key]; ok {
		l.mu.Unlock()
		return func() (*athena.Corporation, error) {
			return it, nil
		}
	}
	if l.batch == nil {
...
func (l *CorporationLoader) LoadAll(keys []uint) ([]*athena.Corporation, []error) {
	results := make([]func() (*athena.Corporation, error), len(keys))

	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}

	corporations := make([]*athena.Corporation, len(keys))
...
func (l *CorporationLoader) unsafeSet(key uint, value *athena.Corporation) {
	if l.cache == nil {
		l.cache = map[uint]*athena.Corporation{}
	}
	l.cache[key] = value
}
