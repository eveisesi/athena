...
func (l *RegionLoader) LoadThunk(key uint) func() (*athena.Region, error) {
	...
	if it, ok := l.cache[key]; ok {
		l.mu.Unlock()
		return func() (*athena.Region, error) {
			return it, nil
		}
	}
	if l.batch == nil {
...
func (l *RegionLoader) LoadAll(keys []uint) ([]*athena.Region, []error) {
	results := make([]func() (*athena.Region, error), len(keys))

	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}

	regions := make([]*athena.Region, len(keys))
...
func (l *RegionLoader) unsafeSet(key uint, value *athena.Region) {
	if l.cache == nil {
		l.cache = map[uint]*athena.Region{}
	}
	l.cache[key] = value
}
