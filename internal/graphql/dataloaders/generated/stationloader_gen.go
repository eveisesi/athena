...
func (l *StationLoader) LoadThunk(key uint) func() (*athena.Station, error) {
	...
	if it, ok := l.cache[key]; ok {
		l.mu.Unlock()
		return func() (*athena.Station, error) {
			return it, nil
		}
	}
	if l.batch == nil {
...
func (l *StationLoader) LoadAll(keys []uint) ([]*athena.Station, []error) {
	results := make([]func() (*athena.Station, error), len(keys))

	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}

	stations := make([]*athena.Station, len(keys))
...
func (l *StationLoader) unsafeSet(key uint, value *athena.Station) {
	if l.cache == nil {
		l.cache = map[uint]*athena.Station{}
	}
	l.cache[key] = value
}
...
func (b *stationLoaderBatch) keyIndex(l *StationLoader, key uint) int {
...
func (b *stationLoaderBatch) startTimer(l *StationLoader) {
	time.Sleep(l.wait)
	l.mu.Lock()
...
func (b *stationLoaderBatch) end(l *StationLoader) {
