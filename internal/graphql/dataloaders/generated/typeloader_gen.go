...
func (l *TypeLoader) LoadAll(keys []uint) ([]*athena.Type, []error) {
	results := make([]func() (*athena.Type, error), len(keys))

	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}

	types := make([]*athena.Type, len(keys))
...
func (l *TypeLoader) unsafeSet(key uint, value *athena.Type) {
	if l.cache == nil {
		l.cache = map[uint]*athena.Type{}
	}
	l.cache[key] = value
}
...
func (b *typeLoaderBatch) keyIndex(l *TypeLoader, key uint) int {
