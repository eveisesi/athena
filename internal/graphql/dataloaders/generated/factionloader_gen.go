...
type FactionLoader struct {
	...
	cache map[uint]*athena.Faction

	// the current batch. keys will continue to be collected until timeout is hit,
	// then everything will be sent to the fetch method and out to the listeners
	batch *factionLoaderBatch

	// mutex to prevent races
	mu sync.Mutex
}

type factionLoaderBatch struct {
	keys    []uint
	data    []*athena.Faction
	error   []error
	closing bool
...
// LoadThunk returns a function that when called will block waiting for a Faction.
...
func (l *FactionLoader) LoadThunk(key uint) func() (*athena.Faction, error) {
	...
	if it, ok := l.cache[key]; ok {
		l.mu.Unlock()
		return func() (*athena.Faction, error) {
			return it, nil
		}
	}
	if l.batch == nil {
		l.batch = &factionLoaderBatch{done: make(chan struct{})}
	}
	batch := l.batch
...
func (l *FactionLoader) LoadAll(keys []uint) ([]*athena.Faction, []error) {
	results := make([]func() (*athena.Faction, error), len(keys))

	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}

	factions := make([]*athena.Faction, len(keys))
	errors := make([]error, len(keys))
	for i, thunk := range results {
		factions[i], errors[i] = thunk()
	}
	return factions, errors
}

// LoadAllThunk returns a function that when called will block waiting for a Factions.
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
...
func (l *FactionLoader) unsafeSet(key uint, value *athena.Faction) {
	if l.cache == nil {
		l.cache = map[uint]*athena.Faction{}
	}
	l.cache[key] = value
}
...
func (b *factionLoaderBatch) keyIndex(l *FactionLoader, key uint) int {
...
func (b *factionLoaderBatch) startTimer(l *FactionLoader) {
	time.Sleep(l.wait)
	l.mu.Lock()
...
func (b *factionLoaderBatch) end(l *FactionLoader) {
