...
type SolarSystemLoader struct {
	...
	cache map[uint]*athena.SolarSystem
...
func (l *SolarSystemLoader) LoadThunk(key uint) func() (*athena.SolarSystem, error) {
	...
	if it, ok := l.cache[key]; ok {
		l.mu.Unlock()
		return func() (*athena.SolarSystem, error) {
			return it, nil
		}
	}
	if l.batch == nil {
...
func (l *SolarSystemLoader) LoadAll(keys []uint) ([]*athena.SolarSystem, []error) {
	results := make([]func() (*athena.SolarSystem, error), len(keys))

	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}

	solarSystems := make([]*athena.SolarSystem, len(keys))
...
func (l *SolarSystemLoader) unsafeSet(key uint, value *athena.SolarSystem) {
	if l.cache == nil {
		l.cache = map[uint]*athena.SolarSystem{}
	}
	l.cache[key] = value
}
...
func (b *solarSystemLoaderBatch) keyIndex(l *SolarSystemLoader, key uint) int {
